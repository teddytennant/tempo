#!/usr/bin/env bash
# Build + packed-smoke + submit the Great Tusk mill-box rules agent (alancai27 replica+improve).
#   bash scripts/ship_tusk.sh "submission message" [--dry-run]
set -euo pipefail
cd /home/gradient/projects/ai/tempo
MSG="${1:?usage: ship_tusk.sh <message> [--dry-run]}"
DRY="${2:-}"
TUSK=data/decks/mined/alancai_tusk.csv
OUT="/home/gradient/projects/ai/tempo/agent/submission_tusk.tar.gz"

# Serialize the whole ship sequence against sibling sessions.
exec 200>/tmp/tempo_ship.lock
flock -x 200

# deck.csv must be the mined tusk list or the tusk specialist never fires.
cp "$TUSK" agent/deck.csv
EXPECT_DECK="$TUSK" CG_LIB_PATH="$(pwd)/cg" bash agent/build_submission.sh
mv agent/submission.tar.gz "$OUT"

# tusk_rules.py must actually be in the tarball (build_submission.sh has a hardcoded module list).
# NB: grep WITHOUT -q — pipefail + grep -q early-exit SIGPIPEs tar into a spurious failure.
if [ "$(tar -tzf "$OUT" | grep -c "tusk_rules.py")" -lt 1 ]; then
  echo "ERROR: tusk_rules.py missing from tarball" >&2; exit 3
fi
