#!/usr/bin/env bash
# Build + packed-smoke + submit the Starmie v11 rules agent (v10 + ladder-loss-driven anti-Dragapult
# Froslass/Refrain plan, from real v10 ladder episode 86501434).
#   bash scripts/ship_starmie_v11.sh "submission message" [--dry-run]
# The WHOLE sequence is serialized against sibling agents via /tmp/tempo_ship.lock (deck.csv is a
# shared file: another specialist shipping concurrently would otherwise race us on it).
set -euo pipefail
cd /home/gradient/projects/ai/tempo
MSG="${1:?usage: ship_starmie_v11.sh <message> [--dry-run]}"
DRY="${2:-}"
DECK=data/decks/starmie.csv
OUT="/home/gradient/projects/ai/tempo/agent/submission_starmie_v11.tar.gz"

exec 200>/tmp/tempo_ship.lock
flock -x 200

# deck.csv must be the Starmie list or the starmie specialist never fires.
