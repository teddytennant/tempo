#!/usr/bin/env bash
# Build + packed-smoke + submit the Grimmsnarl v2 rules agent (Crustle-wall doctrine).
#   bash scripts/ship_grimmsnarl_v2.sh "submission message" [--dry-run]
# The WHOLE sequence is serialized against sibling ship runs via /tmp/tempo_ship.lock.
set -euo pipefail
exec 200>/tmp/tempo_ship.lock
flock -x 200

cd /home/gradient/projects/ai/tempo
MSG="${1:?usage: ship_grimmsnarl_v2.sh <message> [--dry-run]}"
DRY="${2:-}"
TEA=data/decks/mined/The_Debauchery_Tea_Party.csv
OUT="/home/gradient/projects/ai/tempo/agent/submission_grimmsnarl_v2.tar.gz"
