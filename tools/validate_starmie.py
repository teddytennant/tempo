"""Head-to-head: the Starmie-aware scorer vs the SAME scorer with the Starmie specialist disabled.

Both pilot data/decks/starmie.csv; seats alternate each game so neither side gets a permanent
first/second-player edge. The "generic" baseline is just `scorer.best_options` with the module's
`_starmie` hook temporarily set to None — i.e. byte-for-byte the generic scorer path. This avoids
any confound from other specialists or concurrent edits.

No-regression: on data/decks/dragapult.csv and data/decks/crustle.csv, the Starmie hook never fires
(no Starmie line on our side), so enabling vs disabling it must yield IDENTICAL selections on every
decision of a replayed game. We assert exactly that and report the mismatch count (expect 0).

Run:  ./scripts/run.sh -m tools.validate_starmie --games 40
"""
...
def generic_best(obs_dict):
    """best_options with the Starmie specialist disabled -> the generic scorer path."""
    saved = scorer._starmie
    scorer._starmie = None
    try:
        return scorer.best_options(obs_dict)
    finally:
        scorer._starmie = saved
...
def regression_check(deck_path, games=4, max_steps=6000):
    """Replay self-play games; at every decision compare tuned vs generic best_options. The Starmie
...
def main():
    ...
    print(f"\n=== tuned-Starmie vs generic-Starmie: W{w} L{l} D{d}  "
          f"tuned winrate = {wr:.1f}% over {args.games} games ===")

    print("\n--- no-regression: identical selections off-archetype (Starmie hook must not fire) ---")
    for name in ("dragapult.csv", "crustle.csv"):
        try:
            mism, dec = regression_check(os.path.join(_ROOT, "data", "decks", name))
            status = "OK" if mism == 0 else f"REGRESSION ({mism} differ)"
            print(f"{name:14s}: {mism} mismatches / {dec} decisions  [{status}]")
