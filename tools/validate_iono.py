"""Head-to-head: the Iono-aware scorer vs the SAME scorer with the Iono specialist disabled.

Both pilot data/decks/iono.csv (Iono's Bellibolt ex Lightning energy-stacking engine); seats
alternate each game so neither side gets a permanent first/second-player edge. The "generic" baseline
is just `scorer.best_options` with the module's `_iono` hook temporarily set to None — i.e.
byte-for-byte the generic scorer path. This isolates the specialist from other specialists / edits.

No-regression: on data/decks/crustle.csv and data/decks/dunsparce.csv the Iono hook never
fires (no Iono signature Pokémon on our side), so enabling vs disabling it
...
Run:  ./scripts/run.sh -m tools.validate_iono --games 40
"""
...
def generic_best(obs_dict):
    """best_options with the Iono specialist disabled -> the generic scorer path."""
    saved = scorer._iono
    scorer._iono = None
    try:
        return scorer.best_options(obs_dict)
...
def regression_check(deck_path, games=4, max_steps=6000):
    """Replay self-play games; at every decision compare tuned vs generic best_options. The Iono
...
def main():
    ...
    ap.add_argument("--deck", default=os.path.join(_ROOT, "data", "decks", "iono.csv"))
    args = ap.parse_args()
    ...
    print(f"\n=== tuned-Iono vs generic-Iono: W{w} L{l} D{d}  "
          f"tuned winrate = {wr:.1f}% over {args.games} games ===")

    print("\n--- no-regression: identical selections off-archetype (Iono hook must not fire) ---")
    for name in ("crustle.csv", "dunsparce.csv"):
        try:
