...
cp "$SRC/prize_tracker.py" "$SRC/belief.py" "$SRC/opp_detect.py" "$SRC/opp_decks.py" "$SRC/scorer.py" "$SRC/crustle_rules.py" "$SRC/lucario_rules.py" "$SRC/starmie_rules.py" "$SRC/dunsparce_rules.py" "$SRC/iono_rules.py" "$SRC/lethal.py" "$TMP/" 2>/dev/null || true
