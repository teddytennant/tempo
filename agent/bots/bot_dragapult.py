"""Independent opponent: a Dragapult ex spread/aggro bot (skarin, "Phantom Dive or Go Home").

Faithful port of the published notebook
  skarin__phantom-dive-or-go-home-a-dragapult-ex-deck
to tempo's `cg.api`. The notebook was already written against this exact engine, so
the `main_option_proc` + `hand_score`/`attach_score` + `agent()` logic below is a
*verbatim* reproduction of its `main.py` — only the deck-loading was rewired to read
this bot's own decklist (deck_dragapult.csv) and the public entry point was aliased
to `best_options` (with the original `agent` name kept as an alias).

This is a genuinely independent policy and does NOT call tempo's scorer. It is a top
frontier archetype: it counts the opponent's bench, plans Phantom Dive's 6 spread
damage counters to line up multi-prize turns, tracks its own deck/prize composition
to decide what to search/discard, and only attacks once the board is set up.

A different *style* of opponent from the Crustle wall (which just stalls): this one
is a proactive spread-aggro deck, so it stresses our agents on a totally different axis.

Card / attack ids verified against all_card_data() / all_attack():
  119 Dreepy, 120 Drakloak, 121 Dragapult ex, 140 Fezandipiti ex, 184 Latias ex,
  235 Budew, 1071 Meowth ex, attack 154 Phantom Dive, attack 323 Itchy Pollen.

Per-game module state (logs / prize composition / attack plan) is reset by reset()
so a worker that plays several games does not leak one game's plan into the next.
"""
...
_DECK_PATH = os.path.join(os.path.dirname(os.path.abspath(__file__)), "deck_dragapult.csv")
...
# Load all card data from the API's helper function and build an id -> data lookup.
...
def reset():
    """Reset all per-game module state (called by the eval harness between games)."""
    global can_switch, can_attack, can_main_attack, can_energy_attach
    global use_support, bench_attacker, pre_turn_log, current_turn_log
    global prize, card_counts, serial_set, plan_a, plan_b
...
def no_damage_dex(id: int) -> bool:
    """Defending Pokemon with innate immunity to Dragapult ex's main attack."""
    # Drednaw, Milotic ex, Sylveon, Crustle
    return id == 158 or id == 207 or id == 330 or id == 345


def no_damage_counter(pokemon: Pokemon) -> bool:
    """Targets that prevent placement of Phantom Dive's 6 bench damage counters."""
...
def prize_count(pokemon: Pokemon, is_attack_damage: bool) -> int:
    """How many Prize cards a Pokemon yields when Knocked Out, factoring modifiers."""
...
def pokemon_score(pokemon: Pokemon, is_attack_damage: bool) -> int:
    """Heuristic tactical worth of targeting a specific opposing Pokemon."""
    data = card_table[pokemon.id]
...
def best_options(obs_dict: dict) -> list[int]:
    """Main agent function (skarin Dragapult ex bot)."""
